...
data_2023.drop(data_2023.columns[[0, 1]], axis=1, inplace=True)
data_2023.drop(list(data_2023)[95:102], axis=1, inplace=True)
del data_2023['None']  # Prevents errors due to a column being named "None"
...
# For each of the answer columns, the contents are either NaN or the selected choice (i.e. same as the column name). For example, you can inspect column 14:
# print(data_2023[data_2023.columns[14]])
...
questions_map = {
    ...
    "Which sources do you use to find napari plugins?": (66, 76),
...
# Create a new dataframe with processed columns
...
def create_output(questions_map, show_title=False):
    for question, cols in questions_map.items():
        pcols = gencols(cols)
        fig, ax = plt.subplots()
        if show_title:
...
### Which kind of organization(s) are you associated with?

```{code-cell} ipython3
:tags: [remove-input]

create_output(questions_map={
    "Which kind of organization(s) are you associated with?": (0,5),
})
```
...
### Which of these best describes your Python programming experience level?
...
## Your napari experience

### Approximately how long have you used napari?
...
### Which types of images have you worked with during the past year, or plan to work with in upcoming projects?
...
### Indicate the ways in which you have engaged in the napari community
...
### How satisfied or dissatisfied are you with your *experience using napari with plugins*?
...
### How often are you able to find a napari plugin that meets your needs?
...
### If a future version of napari were to introduce opt-in, anonymous data gathering, what would be your preferred course of action?
...
### Overall, how satisfied or dissatisfied are you with napari?
